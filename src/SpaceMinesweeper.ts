...
export class SpaceMinesweeper extends gfx.GfxApp
{
    ...
    update(deltaTime: number): void 
    {
        ...
        this.stars.children.forEach((star: gfx.Transform2) => {
            ...
            // ADD YOUR CODE HERE
            const distance_to = this.mousePosition.distanceTo(this.ship.position);
            if (distance_to > 0.01)
            {
                
                const starsMovement = gfx.Vector2.subtract(this.ship.position, this.mousePosition);
                starsMovement.normalize();
                starsMovement.multiplyScalar(distance_to * star.scale.x * 2);
                star.position.add(starsMovement);
        ...
        this.mines.children.forEach((mine: gfx.Transform2) => {
            ...
            // ADD YOUR CODE HERE
            mine.rotation += Math.PI / 90;
            const distance_to = this.mousePosition.distanceTo(this.ship.position);
            if (distance_to > 0.01)
            {
                
                const mineMovement = gfx.Vector2.subtract(this.ship.position, this.mousePosition);
                mineMovement.normalize();
                mineMovement.multiplyScalar(distance_to*0.02);
    ...
    onMouseDown(event: MouseEvent): void 
    {
        ...
        laserInstance.position.set(0, 0);
    ...
    private checkForMineCollisions(): void
    {
        for(let i=0; i < this.mines.children.length; i++)
        {
            for(let j=i+1; j < this.mines.children.length; j++)
            {
                ...
                // ADD YOUR CODE HERE
                if (mine1.intersects(mine2) || mine2.intersects(mine1)){
                    mine1.explode();
                    mine2.explode();
                }
